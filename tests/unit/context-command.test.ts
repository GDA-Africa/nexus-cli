...
describe('composeContext — the maxTokens adapter over getContextTool', () => {
  ...
  it('a smaller maxTokens produces a pack that fits a proportionally smaller size', async () => {
    ...
    const tiny = await composeContext(ctx, { task: 'mount renames npm', maxTokens: 50 });

    // getContextTool clamps maxChars to a 2000-char floor, so a 50-token
    // request still returns *something* — the point is it is not larger
    // than the generous request, and knowledge (the least-privileged
    // section) is the first to be dropped or truncated.
