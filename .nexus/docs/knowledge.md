...
## [2026-09-07] gotcha — any new JSON-piping command needs the clean-stdout carve-out in cli.ts
`nexus context --json` and `nexus graph --json` pipe machine-readable JSON to stdout. The CLI's `→ Brain: last synced …` line is emitted by the `postAction` auto-invoke hook (and the update banner by the update check), both of which write plain text to the same stdout — silently corrupting a JSON parse. The rule in `src/cli.ts`: every command that writes JSON to stdout through a clean pipe must be added to `needsCleanStdout()` (currently `isMcpInvocation() || isJsonContextInvocation() || isJsonGraphInvocation()`). If you add another `--json`-to-stdout command and it "works" in a terminal but breaks piped into `python3 -m json.tool` / a harness, this is the cause.

## [2026-09-07] gotcha — the "17 tools" doc-comment and MCP tool count drifted twice
The annotated tool surface in `src/mcp/server.ts`, `src/mcp/index.ts`, `NEXUS.md`, and `.nexus/docs/04_api_contracts.md` all said "17 tools", but `tests/unit/mcp-tools.test.ts` had asserted `toHaveLength(18)` — the comment had gone stale when `nexus_plan_verify` was added. Adding `nexus_project_graph` pushed the real count to **19 (15 read, 4 write)**. Lesson: the truth is the `client.listTools()` length in the MCP e2e test, not any doc-comment count. When touching the tool surface, grep for the number across `src/mcp/server.ts`, `src/mcp/index.ts`, `NEXUS.md`, `04_api_contracts.md`, and the MCP test, and reconcile them in one pass.

## [2026-09-07] gotcha — `release:check --fast` skips the README test-badge measurement; unstale it by hand
The full `npm run release:check` derives every advertised number from source and compares against README/site/`llms.txt`; it measures `facts.tests` by running the suite. The `--fast` flag skips that measure, so a stale README badge (found `807_passing` when the real count was 841) sails through a `--fast` run and is only caught by the full run. The badge regex is `/tests-(\d+)_passing/` driven by `facts.tests`. During v1.6.0 prep the badge was corrected to `841_passing`; the full gate confirmed 841 · 23 commands · 19 MCP tools · 16 doctor checks all consistent. Lesson: run the **full** `release:check` (not `--fast`) as the final pre-cut gate, and let it measure the suite.

## [2026-09-07] gotcha — the v1.6.0 feature commits landed on `main` with un-fixed `import/order` lint errors
`nexus log`/`nexus note`/`nexus graph`/`nexus plan verify` and the MCP knowledge-tool unification shipped with six `import/order` errors in `src/cli.ts` (command imports out of order), `src/mcp/server.ts`, `src/mcp/tools.ts`, and `src/utils/brain-memory.ts` (no blank line between import groups). They only surfaced as a FAIL if `npm run lint` (or the release/pre-commit gate) was actually run. `eslint --fix` resolves all six (purely reorders imports, ~10 lines total, no behavior change); `tsc --noEmit` + 841 tests still green after the fix. Only a pre-existing `@typescript-eslint/no-explicit-any` warning at `tools.ts:1048` remains (warning, not error). Lesson: the new-feature commits were not lint-clean when committed; run `npm run lint` before any release cut.

## [2026-09-07] gotcha — `git-sensor.test.ts` "should return valid git info" times out under full-suite parallel load
When the full 63-file suite runs, vitest's parallel workers heavily contend git subprocess I/O and the git-repo test (`git init` → config → commit → checkout → commit → log → captureGitSensor) occasionally exceeded vitest's default 5000ms test timeout, surfacing as `Test timed out in 5000ms` at `git-sensor.test.ts:29`. The same test passed 5/5 in isolation (~1500ms) but failed intermittently in full runs (5000ms+). Fix: pass an explicit `30_000` as `it(name, fn, timeout)` — the assertions are unchanged, so it isn't test-gaming; it just removes the wall-clock cutoff for a git-heavy integration test on slow/sandboxed CI. Notably this surfaced via `release:check`'s `[SUITE] ... failing` because `measureTests()` runs the suite; the only emission of that is in `release-check.mjs`. Lesson: release-gate that runs the full suite is exactly where such a timeout-flake gets caught — fix the per-test timeout rather than loosening assertions.
