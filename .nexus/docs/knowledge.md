...
### [gotcha] D16 shipped catching the exact drift its own commit describes — and immediately caught it
**2026-08-24** — The commit that added D16 (`2908abe`, artifact drift) explains it exists because "§19 counts-in-prose rot... had recurred inside NEXUS's own brain," citing `plans/index.md` omitting open plans and `docs/index.md` describing a stale phase. Thirteen commits landed the same day, including D16 itself, and nobody updated `index.md` or `knowledge.md` to reflect any of them — the exact pattern, on the exact commit batch that shipped the detector for it. A live `nexus doctor` run after this sync still shows D16 firing on `.nexus/plans/index.md` (3 open plans on disk — `release-v1-2`, `implement-v1-2-provable-done`, `bootstrap-nexus-cli-roadmap` — not listed in the dashboard) and D15 firing on the pre-existing `packageManager: yarn` vs. `package-lock.json` mismatch in `.nexus/manifest.json`.
**Why:** A drift detector that ships in the same batch it should have flagged is not evidence the detector works — the batch that adds a check is never checked by it, because nothing re-runs `nexus doctor` against the diff before commit. `--strict` in CI (already landed in v1.3) is what would close that specific gap; it did not run here.
**How to apply:** After any commit batch that touches `.nexus/`, run `nexus doctor` before considering the brain in sync, not just the code's own test suite. Don't assume a newly-shipped drift check has already caught drift in the commits that shipped it — verify by running it. The `plans/index.md` and manifest mismatches found here were deliberately left unfixed by this sync (out of scope for a brain-content update); a future session should either run `nexus plan` commands to regenerate the dashboard or hand-reconcile it, and correct `.nexus/manifest.json`'s `packageManager`.

### [convention] Two files named knowledge.md exist; only `.nexus/docs/knowledge.md` is live
**2026-08-24** — A stale top-level `.nexus/knowledge.md` (last touched at the v0.3.2 release, March 2026, still using the old `## [date] category — title` heading format) sits alongside this file, `.nexus/docs/knowledge.md`, which is the one every instruction file, `src/utils/knowledge.ts`'s `ENTRY_HEADING` parser, and `nexus consolidate` actually read and write. The two have diverged since v0.3.2 — the top-level copy is missing every entry added since, including this one.
**Why:** Nothing in the generators writes or reconciles `.nexus/knowledge.md` (confirmed: `grep -rn "knowledge.md" .nexus/ai/instructions.md CLAUDE.md AGENTS.md` only ever points at `.nexus/docs/knowledge.md`) — it appears to be a leftover from before the file moved under `docs/`, per the `[2026-02-09] gotcha` entry already in this log about that exact move.
**How to apply:** If a task description or an orchestrator's instructions reference `.nexus/knowledge.md` (no `docs/`), treat that as the stale duplicate and write to `.nexus/docs/knowledge.md` instead — cross-check `ENTRY_HEADING` in `src/utils/knowledge.ts` and this file's own most recent real entries before trusting any path claim about where the log lives.

### [architecture] Harness profiles: a smaller-budget instruction file is a different file, not a truncation
**2026-08-24** — `.nexus/harnesses.yml` (optional; absent it, generation is byte-identical to before) declares each target's `window`, `orientation_budget`, and `tool_calling` reliability. `toolInstructionContent` (`src/generators/ai-config.ts`) now dispatches per profile: `tool_calling: native` with room to spare gets the unchanged standard file; `native` but over budget gets a ~300–600 byte native-pointer variant whose entire content is "call `nexus_get_context` first, do not read the two brain files directly"; `unreliable`/`none` gets a static-fallback variant with invariants and `knowledge-summary.md` inlined (never mid-entry truncated) and no MCP dependency at all. Each generated file ends with a structural `<!--nexus-reads:...-->` marker recording which brain files its own protocol tells the agent to read, which D14's project-total check decodes instead of re-deriving generation's variant logic or grepping prose.
**Why:** An earlier finding (this log, 2026-02-09-era entries and NEXUS.md §13) says cross-file pointers get ignored by weaker models and is why every tool file used to be self-contained — but that finding was about a pointer competing with the rest of an 8KB file for attention, not about pointers as such. A pointer that *is* the entire file doesn't have that problem, so shrinking the standard file by 80% and inverting the protocol's contradiction (P0.6) are two different fixes that had to land together.
**How to apply:** Don't "slim" the standard instruction template further to hit a smaller budget — that reproduces the diluted-pointer failure mode. A genuinely small budget needs a structurally different variant (pointer-only or static-fallback), selected by `harnesses.yml`, not a shorter copy of the same template.
